...
class CreatFiles {
	constructor(config) {
		this.srcPath = path.join(__dirname, '..', config.includes[0]).replace(/\\/g, '/').replace(/(\/node_modules)?\/vue-src-to-specs/, '');
		this.specsPath = path.join(__dirname, '..', config.output).replace(/\\/g, '/').replace(/(\/)?\/vue-src-to-specs/, '');
		...
		let excludes = this.includes.map(ele => {
			...
			if (ele.indexOf('!') === 0) {
				if (path.basename(ele).indexOf('.') !== -1) { // 文件
					...
					filesReg = new RegExp(ele.replace(/!/, '').replace(/(\*\*)?\/\*\*?/g, '') + '+');
				}
	...
	readDir(readSrcPath) {
		...
		fs.readdir(readSrcPath, {
			...
			if (!err) {
				files.map(ele => {
					if (this.isDirExclude(readSrcPath, ele)) return;
					// 判断是否是文件夹
